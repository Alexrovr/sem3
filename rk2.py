...
def initialize_data():
    ...
    return databases, tables, tables_databases


def build_one_to_many(databases, tables):
    return [(t.name, t.row_count, d.name)
            for d in databases
            for t in tables
            if t.db_id == d.id]

def build_many_to_many(databases, tables, tables_databases):
...
def query1_sorted_tables(databases, tables):
    one_to_many = build_one_to_many(databases, tables)
    return sorted(one_to_many, key=lambda i: i[2])

def query2_databases_with_row_sum(databases, tables):
    one_to_many = build_one_to_many(databases, tables)
    ...
    return sorted(res2_unsorted, key=lambda i: i[1], reverse=True)

def query3_databases_with_base_in_name(databases, tables, tables_databases):
    many_to_many = build_many_to_many(databases, tables, tables_databases)
...
def main():
    ...
    res1 = query1_sorted_tables(databases, tables)
    res2 = query2_databases_with_row_sum(databases, tables)
    res3 = query3_databases_with_base_in_name(databases, tables, tables_databases)
